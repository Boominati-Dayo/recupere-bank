...
const TrackClaimPage = () => {
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'approved': return 'text-navy-900 bg-primary-500 border-primary-600/20';
            ...
            <div className="container mx-auto px-4">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="text-center mb-16 space-y-4">
                        ...
                        >
                            TRACK YOUR <span className="text-primary-600">CLAIM</span>
                        </motion.h1>
                        <motion.p
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
                        ...
                        >
                            <form onSubmit={handleTrack} className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full px-6 py-5 pl-14 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary-500/20 text-navy-950 font-medium placeholder:text-gray-300"
                                            value={email}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full px-6 py-5 pl-14 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-primary-500/20 text-navy-950 font-black tracking-widest uppercase placeholder:text-gray-300 transition-all focus:bg-white"
                                            value={claimNumber}
                                ...
                                <button
                                    disabled={loading}
                                    type="submit"
                                    className="w-full py-5 bg-navy-900 text-primary-500 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-navy-800 transition-all shadow-xl shadow-navy-900/10 flex items-center justify-center gap-3 active:scale-[0.98] disabled:opacity-70"
                                >
                                    {loading ? (
                            ...
                            <div className="mt-8 pt-8 border-t border-gray-50 text-center">
                                ...
                                <button className="text-primary-600 font-black text-xs uppercase tracking-widest mt-2 hover:underline">Contact Intelligence Support</button>
                            </div>
                        </motion.div>
                    ) : (
                        <motion.div
                        ...
                        >
                            ...
                            <div className="bg-navy-900 rounded-[40px] p-8 mobile:p-12 text-white relative overflow-hidden shadow-2xl">
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10">
                                    <div>
                                        <div className="flex items-center gap-3 mb-2">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-primary-500">Forensic Briefing</span>
                                ...
                                {caseData.status === 'completed' && (
                                    <div className="mt-12 p-6 rounded-3xl bg-primary-500 text-navy-950 flex flex-col md:flex-row items-center justify-between gap-6 anime-in bounce-in">
                                        ...
                                        <button onClick={() => window.location.href = '/signup'} className="w-full md:w-auto px-8 py-3 bg-navy-900 text-primary-500 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-navy-800 transition-all">
                                            Finalize Withdrawal
                                        </button>
                                    </div>
                                )}
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-6 gap-3 mobile:gap-4">
                                {getStatusSteps(caseData.status).map((step, idx) => (
                                    <div key={idx} className={`p-6 rounded-[2rem] border transition-all ${step.status === 'active'
                                        ...
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${step.status === 'active' ? 'bg-navy-900 text-primary-500' :
                                            ...
                                            <div key={i} className="relative pl-16 lg:pl-24 group">
                                                <div className={`absolute left-4 lg:left-6 -translate-x-1/2 w-4 h-4 rounded-full border-4 border-white transition-all duration-500 z-10 
                                                    ${caseData.status === 'completed' ? 'bg-emerald-500' :
                                                        i === 0 ? `${getStatusDotColor(update.status)} scale-125 shadow-lg` : 'bg-gray-200 group-hover:bg-primary-500/50'}
                                                    `}></div>
                                                ...
                                                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                                                    ...
                                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest pt-1 shrink-0">
                                                        {new Date(update.timestamp).toLocaleString('en-AU', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                                                    </p>
                                                </div>
                                            </div>
                                        ))
