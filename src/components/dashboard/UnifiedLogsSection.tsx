...
const UnifiedLogsSection = () => {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'withdrawal': return <ArrowUpCircle className="w-5 h-5 text-[#ee2737]" />;
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'failed': return <XCircle className="w-4 h-4 text-[#ee2737]" />;
        ...
        <div className="block md:hidden space-y-3">
          {loading ? (
            <div className="text-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#ee2737] mx-auto mb-4"></div>
              <p className="text-gray-600">Loading transactions...</p>
            </div>
          ) : error ? (
            <div className="text-center py-8 text-[#ee2737]">
              <p>{error}</p>
              <button
                onClick={fetchTransactions}
                className="mt-2 px-4 py-2 bg-[#ee2737] text-white rounded-lg hover:bg-[#d11f2d]"
              >
                Retry
              </button>
            </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="text-center py-8">
                    <div className="flex items-center justify-center space-x-2">
                      <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-[#ee2737]"></div>
                ...
                <tr>
                  <td colSpan={6} className="text-center py-8 text-[#ee2737]">
                    <p>{error}</p>
                    <button
                      onClick={fetchTransactions}
                      className="mt-2 px-4 py-2 bg-[#ee2737] text-white rounded-lg hover:bg-[#d11f2d]"
